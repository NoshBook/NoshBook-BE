...
module.exports = class RatingsService {
  ...
  static async applyEdit(userId, recipeId, recipeData) {
    ...
    try {
      ...
      if(recipe.ownerId !== userId) {
        // Make the new recipe hidden
        recipeData.isPublic = false;
        // Make the duplicate and store the new Id
  ...
  static async addNew(userId, recipeData) {
    const newId = await Recipe.insertOne(userId, recipeData);
    await CookBook.addRecipe({ recipeId: newId, userId });
    return newId;
  }
