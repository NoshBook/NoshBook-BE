const { calcNewRating } = require('../lib/utils/incrementalAverage.js');
...
describe('incrementalAverage test', () => {
  ...
  it('should average 5 and 5 correctly', () => {
    ...
    const actual = calcNewRating(5, 1, 5).updatedRating;
    expect(actual).toEqual(expected);
  });

  it('should average 5 5 5 5 5 correctly', () => {
    const expected = normalAverage([5, 5, 5, 5, 5]);
    const actual = calcNewRating(5, 4, 5).updatedRating;
    expect(actual).toEqual(expected);
  });

  it('should average 1 2 3 4 5 correctly', () => {
    const expected = normalAverage([1, 2, 3, 4, 5]);
    const actual = calcNewRating(2.5, 4, 5).updatedRating;
    expect(actual).toEqual(expected);
  });

  it('should average 2 2 3 2 5 correctly', () => {
    const expected = normalAverage([2, 2, 3, 2, 5]);
    const actual = calcNewRating(2.25, 4, 5).updatedRating;
    expect(actual).toEqual(expected);
  });

  it('should average 5 correctly', () => {
    const expected = normalAverage([5]);
    const actual = calcNewRating(0, 0, 5).updatedRating;
    expect(actual).toEqual(expected);
  });
});
